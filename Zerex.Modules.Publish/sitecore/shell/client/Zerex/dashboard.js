define(["sitecore", "jquery"], function (sitecore, $) {
	var selectedSite = function(model){
		var app = this;
		
		if (typeof model !== "undefined" && model !== null && model !== "") {
			app.SmartPanel.set("model", model);
			updateSmartPanelData();
			app.SmartPanel.set("isOpen", true);
		}
	};
	
	var updateSmartPanelData = function (){
		var model = Sitecore.Speak.app.SmartPanel.get("model");
		
		if ((typeof model !== "undefined") && (model !== null)) {
			Sitecore.Speak.app.SmartPanelHeader.set("text", model.get("Name"));			
		}
	};
	...
	var app = sitecore.Definitions.App.extend({
		initialized: function () {
			...
			//this.PublishButton.on("click", this.triggerPublish, this);
